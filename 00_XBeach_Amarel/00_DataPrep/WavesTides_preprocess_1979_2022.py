...
def infill_missing_data(x_1, x_2, y_obs, x_1_missing, x_2_missing):
	output_tmp = numpy.zeros(len(x_1_missing)) * numpy.nan
	w_nan = numpy.where(numpy.isnan(x_2_missing)==False)[0]
	y_predicted_lr, y_predicted_train_lr = infill_missing_data_lr(
		x_1, x_2, y_obs, x_1_missing[w_nan], x_2_missing[w_nan]
	)
	...
	X_missing = numpy.column_stack((x_1_missing[w_nan], x_2_missing[w_nan]))
	...
	output_tmp[w_nan] = y_final_predicted
	...
	return output_tmp, y_final_predicted2

def fetch_multi_1_data(station_id='44025', start_date='2010-01', end_date='2019-12'):
    if len(start_date) == 7:
    ...
    for dt_month in rrule.rrule(rrule.MONTHLY, dtstart=start_dt, until=end_dt):
        ...
        base_url = f"https://polar.ncep.noaa.gov/waves/hindcasts/multi_1/{yyyymm}/points"
...
def fetch_nopp_phase2_data(station_id='44025', start_date='1979-01', end_date='2009-12'):
    if len(start_date) == 7:
        start_dt = datetime.datetime.strptime(start_date, "%Y-%m")
    else:
        start_dt = datetime.datetime.strptime(start_date, "%Y-%m-%d")
    ...
    for dt_month in rrule.rrule(rrule.MONTHLY, dtstart=start_dt, until=end_dt):
        yyyymm = dt_month.strftime('%Y%m')
        base_url = f"https://polar.ncep.noaa.gov/waves/hindcasts/nopp-phase2/{yyyymm}/points/buoys"
...
def convert_df_to_wave_data(df):
    ...
    for i, (timestamp, row) in enumerate(df.iterrows()):
        ...
        wave_data[i, 4] = 0.0
        wave_data[i, 8] = row['SWH']
        wave_data[i, 9] = row['Period']
        wave_data[i,11] = row['MWD']
    return wave_data
...
base_url = ("https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?product=hourly_height&"
            "application=NOS.COOPS.TAC.WL&begin_date={year}0101&end_date={year}1231&datum=MLLW&"
            "station={station_num}&time_zone=LST&units=english&format=csv")
...
base_url = ("https://www.ndbc.noaa.gov/view_text_file.php?filename={buoy_num}h{year}.txt.gz&"
...
def process_tide(tide_data):
    ...
    date_list = []
    for n2 in range(len(tide_data[:,0])):
...
def process_wave(wave_data, tide_output_tmp):
    ...
    for n2 in range(len(wave_data[:,0])):
    ...
    wave_output_tmp[w_tmp4,1] = loess2D.weighted_linear(
        time_wave[w_tmp4], time_wave[w_tmp4]*0.0,
        wave_output_tmp_2[w_tmp2,0], wave_output_tmp_2[w_tmp2,0]*0.0,
        wave_output_tmp_2[w_tmp2,2], 3, 3.0/24., 3.0/24.
    )[2]
    wave_output_tmp_linint[w_tmp4,2] = f_period(time_wave[w_tmp4])
    wave_output_tmp[w_tmp4,2] = loess2D.weighted_linear(
        time_wave[w_tmp4], time_wave[w_tmp4]*0.0,
        wave_output_tmp_2[w_tmp3,0], wave_output_tmp_2[w_tmp3,0]*0.0,
        wave_output_tmp_2[w_tmp3,3], 3, 3.0/24., 3.0/24.
    )[2]
    ...
    date_list = []
    for n2 in range(len(wave_output_tmp[:,0])):
...
hourly_timeseries = numpy.arange(time_tmp_first, time_tmp_last+1/24., 1./24.)
...
tide_output_2 = numpy.zeros((len(hourly_timeseries), len(tide_output[0,:]))) * numpy.nan
tide_output_2[:,0] = hourly_timeseries

for n in range(len(tide_output_2[:,0])):
    time_tmp = tide_output_2[n,0]
    w_tidematch = numpy.where((numpy.absolute(tide_output[:,0]-time_tmp)<=0.5/24.))[0]
    if len(w_tidematch) > 0:
        tide_output_2[n,1] = tide_output[w_tidematch,1]

tide_output = tide_output_2 * 1.0
print(tide_output.shape)

def convert_seconds_to_real_time(seconds, start_time):
    return [start_time + datetime.timedelta(days=s) for s in seconds]

date_list_tide = convert_seconds_to_real_time(tide_output[:,0], time_datum_start)
date_list_tide = numpy.array(date_list_tide)
...
'''
df_nopp2 = fetch_nopp_phase2_data(station_id='44025',start_date='1979-01',end_date='2009-12')
print(df_nopp2.head())
print(df_nopp2.tail())
print("Total rows:", len(df_nopp2))

df_data = fetch_multi_1_data(station_id='44025',start_date='2008-01',end_date='2022-12')
print(df_data.head())
print(df_data.tail())
...
wave_output_2 = numpy.zeros((len(hourly_timeseries), len(wave_output[0,:]))) * numpy.nan
wave_output_2[:,0] = hourly_timeseries

for n in range(len(wave_output_2[:,0])):
    time_tmp = wave_output_2[n,0]
    w_tidematch = numpy.where((numpy.absolute(wave_output[:,0]-time_tmp)<=0.5/24.))[0]
    if len(w_tidematch) > 0:
        wave_output_2[n,1:] = wave_output[w_tidematch,1:]

w_nan = numpy.where(numpy.isnan(wave_output_2[:,1]))[0]
wave_output_2[w_nan,1] = numpy.nanmean(wave_output_2[:,1]) + numpy.random.normal(0,numpy.nanstd(wave_output_2[:,1]),size=len(w_nan))
wave_output_2[w_nan,2] = numpy.nanmean(wave_output_2[:,2]) + numpy.random.normal(0,numpy.nanstd(wave_output_2[:,2]),size=len(w_nan))

w_zero = numpy.where(wave_output_2[:,1]<0)[0]
wave_output_2[w_zero,1] = 0.0
w_zero = numpy.where(wave_output_2[:,2]<0)[0]
wave_output_2[w_zero,2] = 0.0

wave_output = wave_output_2 * 1.0
print(wave_output.shape)
...
tide_output_TB_2 = numpy.zeros((len(hourly_timeseries), len(tide_output_TB[0,:]))) * numpy.nan
tide_output_TB_2[:,0] = hourly_timeseries

for n in range(len(tide_output_TB_2[:,0])):
    time_tmp = tide_output_TB_2[n,0]
    w_tidematch = numpy.where((numpy.absolute(tide_output_TB[:,0]-time_tmp)<=0.5/24.))[0]
    if len(w_tidematch) > 0:
        tide_output_TB_2[n,1] = tide_output_TB[w_tidematch,1]

tide_output_TB = tide_output_TB_2 * 1.0
print(tide_output_TB.shape)
...
print(tide_output.shape, tide_output_TB.shape)
...
print()
w_nan_SH = numpy.where((numpy.isnan(tide_output[:,1])))[0]
tide_output[w_nan_SH,1] = numpy.nanmean(tide_output[w_nan_SH,1]) + numpy.random.normal(0,numpy.nanstd(tide_output[w_nan_SH,1]),size=len(w_nan_SH))
...
ax1.set_xlim(date_list_tide[0], date_list_tide[-1])
plt.tight_layout()
plt.savefig('Wave_Tide.png')
print("Data processed")

date_list_tide_str = []
for dt in date_list_tide:
