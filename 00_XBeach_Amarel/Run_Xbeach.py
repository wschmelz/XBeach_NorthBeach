...
for n in range(iters):
	...
	files_to_copy = ["Analysis_DataPrep_20240117.py", "cmep_xbeach.py", "loess2D.py", "plot_profile.py"]
	for file in files_to_copy:
		shutil.copy(os.path.join(wkspc_Analysis, file), analysis_folder)

	os.chdir(wkspc)
	os.chdir(analysis_folder)
	analysis_dataprep_script = "Analysis_DataPrep_20240117.py"
	os.system(f"python {analysis_dataprep_script}")
	print ("go2")
